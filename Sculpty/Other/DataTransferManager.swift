...
class DataTransferManager {
    ...
    func exportExercises(from context: ModelContext) -> Data? {
        do {
            let exercises = try context.fetch(FetchDescriptor<Exercise>())
            ...
            return try encoder.encode(exerciseDTOs)
        } catch {
            debugLog("Error exporting exercises: \(error.localizedDescription)")
            return nil
        }
    }
    
    func exportWorkouts(from context: ModelContext) -> Data? {
        do {
            let workouts = try context.fetch(FetchDescriptor<Workout>())
            ...
            let encoder = JSONEncoder()
            encoder.dateEncodingStrategy = .iso8601
            return try encoder.encode(workoutDTOs)
        } catch {
            debugLog("Error exporting workouts: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    func importAllData(from data: Data, into context: ModelContext, preserveDefaultExercises: Bool = true, defaultExerciseIDs: Set<UUID> = [], importSettings: Bool = true) throws {
        ...
        try clearAllData(in: context, preserveDefaultExercises: preserveDefaultExercises, defaultExerciseIDs: defaultExerciseIDs)
        ...
        for exercise in filteredExerciseModels {
        ...
        for measurementDTO in appData.measurements {
            let measurement = measurementDTO.toModel()
    ...
    @MainActor
    func importExercises(from data: Data, into context: ModelContext) throws {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        
        let exerciseDTOs = try decoder.decode([ExerciseDTO].self, from: data)
        let exercises = exerciseDTOs.map { $0.toModel() }
    ...
    @MainActor
    func importWorkouts(from data: Data, into context: ModelContext) throws {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        ...
        let workoutDTOs = try decoder.decode([WorkoutDTO].self, from: data)
        let workouts = workoutDTOs.map { $0.toModel(exerciseMap: exerciseMap) }
    ...
    // MARK: - Helper Methods
    @MainActor
    func clearAllData(in context: ModelContext, preserveDefaultExercises: Bool = true, defaultExerciseIDs: Set<UUID> = []) throws {
        do {
